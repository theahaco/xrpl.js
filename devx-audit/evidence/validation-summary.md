...
- Request/command response inference follows explicit `api_version`, not mutations to `client.apiVersion`. Nondefault client-version callers must supply the request version explicitly pending a broader client-version contract. See the current follow-up.
