...
describe('client.request', function () {
  ...
  it('accepts documented AMM ledger selectors without allowing misspellings', function () {
    // The amm_info protocol reference documents both selectors. These calls
    // are compile-time checks and do not contact the mock server.
    async function ammLedgerSelection(client: Client): Promise<void> {
      await client.request({
        command: 'amm_info',
        amm_account: 'rExample',
      ...
      await client.request({
        // @ts-expect-error -- a misspelled ledger selector is still rejected
        command: 'amm_info',
        amm_account: 'rExample',
        ledger_indx: 'validated',
      })
    }
    assert.isFunction(ammLedgerSelection)
  })
